		...
		Pass
		{
			...
			fixed noiseSpereA(float zoom, fixed3 subnoise, float anim)
			{
				...
				if (sphere <sqRadius) {
					...
					if (_Detail>0.0) s = noise4d(fixed4(vs*3.6864 + subnoise, fragTime*_SpeedHi))*0.625;
					if (_Detail>1.0) s =s*0.85+noise4d(fixed4(vs*61.44 + subnoise*3.0, fragTime*_SpeedHi*3.0))*0.125;
			...
			fixed noiseSpereB(float zoom, fixed3 subnoise, float anim)
			{
				...
				if (sphere <sqRadius) {
					fixed3 vs=surfase*zoom;
					if (_Detail>0.0) s = noise4d(fixed4(vs*3.6864 + subnoise, fragTime*_SpeedHi))*0.625;
					if (_Detail>1.0) s =s*0.85+noise4(fixed4(vs*61.44 + subnoise*3.0, fragTime*_SpeedHi*3.0))*0.125;
					if (_Detail>2.0) s =s*0.94+noise4(fixed4(vs*307.2 + subnoise*5.0, anim*5.0))*0.0625;//*0.03125;
					//if (_Detail>3.0) s =s*0.98+noise4(fixed4(vs*600.0 + subnoise*6.0, anim*6.0))*0.03125;
					//if (_Detail>4.0) s =s*0.98+noise4(fixed4(vs*1200.0 + subnoise*9.0, anim*9.0))*0.01125;
				}
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				sphere = dot(posGlob.xyz, posGlob.xyz) - RayProj*RayProj;
				...
				if (sqDist<=sqRadius) {
					...
					fixed3 pr = ray*RayProj - posGlob.xyz;
